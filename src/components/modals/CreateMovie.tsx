...
const CreateMovie = () => {
  ...
  const onSubmit = form.handleSubmit(
    async (values: z.infer<typeof movieSchema>) => {
      try {
        setLoading(true);
        let req;
        if (movie) {
          await axios
            .put(`/api/movies/${movieId}`, { ...values, id: movie.id })
            .then((res) => {
  ...
  return (
    <Modal isVisible={isOpen} onClose={onClose} disable={isLoading}>
      <form
        onSubmit={onSubmit}
        className="p-5 w-screen max-w-full h-screen md:h-[70vh] pt-5 md:w-[500px] lg:w-[650px] flex flex-col gap-8 "
      >
        {!moviesLoading ? (
          <>
            <Input
